<template>
    <div class="sortShopping">
        <template v-for="(item, index) in sortShop.slice(0, cursor)">
            <div class="info">
        ...
        <button class="load" @click="addCursor()" v-show="cursorShow">加载更多</button>
    </div>
</template>
...
const goToDetail = (item) => {
    const itemString = JSON.stringify(item); // 序列化为字符串
    router.push({ path: `/shoppingDetail/${item.id}`, params: { item: itemString } });
};
</script>
