    ...
    <div class="sortClass">
        ...
        <div class="right" v-if="classPhone">
            <sort-class-right v-for="(item, index) in sidebarItems" :key="index" ref="sortRefs[item.ref]"
                :title="item.title" :data="getClassData(item.ref)" />
        </div>
    </div>
</template>
...
const getClassData = (className) => {
    return className === 'classPhone' ? classPhone :
        className === 'classCostume' ? classCostume :
            className === 'classDepartment' ? classDepartment :
                className === 'classSports' ? classSports :
                    className === 'classFood' ? classFood :
                        classFruit;
};
</script>
