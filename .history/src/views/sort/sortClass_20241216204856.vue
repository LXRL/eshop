    ...
    <div class="sortClass">
        <div class="left">
            <van-sidebar v-model="active">
                <van-sidebar-item v-for="(item, index) in categories" @click="srcollTo" :key="index" :title="item" />
            </van-sidebar>
        </div>
        <div class="right" v-if="activeClassData.length">
            <sort-class-right v-for="(item, index) in activeClassData" ref="sortItems" :key="index" :title="item.title"
                :data="item.data" />
        </div>
    </div>
</template>
